(function() {
  ...
  TopMasterView = (function(superClass) {
    ...
    TopMasterView.prototype.template = Handlebars.compile('<div class="row"> <div class="col-md-12 col-xs-12 col-sm-12"> <div class="search-header-wrap"> <div class="row breadcrumb-bar"> <div class="col-xs-12 col-md-12"> <div class="bread-crumb-list"> <ul class="brdcrmb-wrp clearfix"> <li class=""> <span class="bread-crumb-current"> <span class=".icon-arrow-right2"></span><a class="unit_back" href="#"> Back to Project Overview</a> </span> </li> </ul> </div> </div> </div> <h1 class="pull-left proj-name">{{project_title}}</h1> <div class="proj-type-count"> {{#types}} <<<<<<< HEAD <h1 class="text-primary pull-left">{{count.length}}</h1> <p class="pull-left">{{type}}</p> {{/types}}<br/> {{#each  filters}}<br/> {{@key}}: {{#each this}}<br/>{{@key}}:{{this}}{{/each}} {{/each }}<br/>{{#each status}}{{@key}}:{{this}}{{/each}} ======= <!--<h1 class="text-primary pull-left">{{count.length}}</h1> <p class="pull-left">{{type}}</p>--> {{/types}} {{#each  filters}} <h1 class="text-primary pull-left">{{#each this}}{{this}}{{/each}}</h1> <p class="pull-left">{{@key}}</p> {{/each }} >>>>>>> 9a1468af25b143d3107d508dc91a772cb27fa424 <div class="clearfix"></div> </div> <button class="btn btn-primary cf-btn-white pull-right m-t-25" type="button" data-toggle="collapse" data-target="#collapsefilters"> Filter </button> <div class="clearfix"></div> </div> </div> </div>');
