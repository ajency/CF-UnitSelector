(function() {
  ...
  LeftView = (function(superClass) {
    ...
    LeftView.prototype.template = Handlebars.compile('<div class="col-md-3 col-xs-12 col-sm-12 search-left-content"> <div class="filters-wrapper"> <div class="tab-main-container"> <div class="blck-wrap"> <h3><strong>{{i10n "project_by"}}</strong></h3> <img src="{{logo}}" class="img-responsive builder-logo"> </div> <div class="proj-details blck-wrap no-hover"> <h3><strong>{{i10n "project_details"}}</strong></h3> <div class=""> <span class="icon-map-marker"></span> <strong>Address: </strong><br> {{address}} </div> <div class="detail-pts"> {{#propertyTypes}} <h4 class="m-b-5 m-t-0 text-primary">{{prop_type}}</h4> <!--  <span>{{i10n "project_type"}}:</span> {{prop_type}} <p> <span>{{i10n "starting_area"}}:</span> {{starting_area}} Sq.Ft. </p>--> <p> <span>{{i10n "unit_types"}}:</span> {{unit_types}} </p> <!--<p> <span>Available:</span> {{#availability}} {{count}}	{{status}} {{/availability}} </p>--> <!--<p> <span>{{i10n "starting_price"}}:</span>  {{starting_price}} </p>--> {{/propertyTypes}} </div> </div> <div class="blck-wrap"> <div class="text-center"> <img src="../images/marker-img.png" class="img-responsive marker-img"> {{i10n "know_your_neighbour"}} </div> </div> </div> </div> </div>');
